...
class AlertData {
  ...
  bool get isSevereAlert {
    final text = '$title $description'.toLowerCase();
    return text.contains('severe') ||
        text.contains('warning') ||
        text.contains('storm') ||
        text.contains('thunder') ||
        text.contains('tornado') ||
        text.contains('hail') ||
        text.contains('extreme') ||
        text.contains('wind');
  }

  bool startsBefore(DateTime dateTime) => start.isBefore(dateTime);

  factory AlertData.fromJson(Map<String, dynamic> json) {
    final eventName = json['event'] ?? 'Alert';
    final desc =
        json['description'] ?? json['sender_name'] ?? 'Weather alert issued';

    // Parse start time
    final startDt = DateTime.fromMillisecondsSinceEpoch(
      (json['start'] as int) * 1000,
    );
    final endDt = DateTime.fromMillisecondsSinceEpoch(
      ((json['end'] as int?) ?? (json['start'] as int)) * 1000,
    );
    ...
    final isToday =
        ...
        startDt.day == now.day;
    final timeStr =
        (isToday ? 'Today ' : '${DateFormat('EE').format(startDt)} ') +
    ...
    if (lowerEvent.contains('storm') ||
        lowerEvent.contains('thunder') ||
        lowerEvent.contains('tornado')) {
    ...
    } else if (lowerEvent.contains('rain') ||
        lowerEvent.contains('flood') ||
        lowerEvent.contains('water')) {
      ...
      timeColor = const Color(0xFF93C5FD);
        ...
        lowerEvent.contains('sun') ||
        lowerEvent.contains('fire')) {
      ...
      timeColor = const Color(0xFFFDBA74);
        ...
        lowerEvent.contains('cold') ||
        lowerEvent.contains('winter')) {
